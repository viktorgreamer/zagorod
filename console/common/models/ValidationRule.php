...
class ValidationRule extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['preg_match', 'name'], 'string', 'max' => 256],
        ];
    }
    ...
    {
        if (($this->required) && (empty($value))) return self::IS_EMPTY;
    ...
    function renderRule($response = '')
    {
        ...
        switch ($this->type) {
            case self::STRING_TYPE:
                {
                    $rule = "Строка длинною от " . $this->min . " до " . $this->max;
                    break;
                }
            case self::INTEGER_TYPE:
                {
                    $rule = "Целое число в диапазоне от " . $this->min . " до " . $this->max;
                    break;
                }
            case self::FLOAT_TYPE:
                {
                    $rule = "Число в диапазоне от " . $this->min . " до " . $this->max;
                    break;
                }
            case self::EMAIL_TYPE:
                {
